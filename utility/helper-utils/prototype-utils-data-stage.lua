--[[
    ...
    These were started to be documented, but it was just too painful and so it was stopped.
...
---@field ingredients? table<string, data.IngredientPrototype>
---@field normal? table<string, data.IngredientPrototype>
---@field expensive? table<string, data.IngredientPrototype>
...
PrototypeUtils.GetRecipeIngredientsAddedTogether = function(recipeIngredientHandlingTables)
    ---@diagnostic disable: no-unknown # Sumneko just has to skip the missing types in this function until its refactored. See note at top of file.
    ...
    for ingredientType in pairs(ingredientTypes) do
        ...
        for _, recipeIngredientHandlingTable in pairs(recipeIngredientHandlingTables) do
            local ingredients ---@type table<string, data.IngredientPrototype> # Try to find the correct ingredients for our desired type, if not found just try all of them to find one to use. Assume its a simple ingredient list last.

            if recipeIngredientHandlingTable[1][ingredientType] ~= nil then
                ingredients = recipeIngredientHandlingTable[1][ingredientType].ingredients or recipeIngredientHandlingTable[1][ingredientType] --[[@as table<string, data.IngredientPrototype>]]
            ...
            elseif recipeIngredientHandlingTable[1]["normal"] ~= nil then
                ingredients = recipeIngredientHandlingTable[1]["normal"].ingredients --[[@as table<string, data.IngredientPrototype>]]
            elseif recipeIngredientHandlingTable[1]["expensive"] ~= nil then
                ingredients = recipeIngredientHandlingTable[1]["expensive"].ingredients --[[@as table<string, data.IngredientPrototype>]]
            else
    ...
    ---@diagnostic enable: no-unknown # Sumneko just has to skip the missing types in this function until its refactored. See note at top of file.
end
...
---@param itemName data.ItemID|string
---@return boolean
...
---@param technologies data.TechnologyPrototype[]
---@return data.TechnologyPrototype[] technologiesChanged # An array of the technologies affected or a blank array if no technologies are affected.
PrototypeUtils.RemoveEntitiesRecipesFromTechnologies = function(entityPrototype, recipes, technologies)
    local technologiesChanged = {} ---@type data.TechnologyPrototype[]
    local placedByItemName ---@type data.ItemID
...
---@param entityToClone data.EntityPrototype
